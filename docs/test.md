...
- **UI 截图基线失败（toHaveScreenshot 断言错误）**：如果因为页面的预期变更（如版权年份更新、文案修改、合理的布局调整等）导致 UI 测试因为像素差异断言失败，且手动检查页面无逻辑问题，此时属于**正常的 UI 基线过期**。请**不要去试图修改正常的页面代码去迎合过期的截图**，直接运行 `npm run baseline:ui` 更新最新的页面截图以覆盖过期基线即可。
- **缺失或凭据校验无响应（AI 卡死）**：自动化测试必须使用 `tests/credentials.json`。在无人值守环境（或交由 AI 处理时），必须在开始前提早检查凭据存不存在，并采取无交互挂载或运行前置 `test:auto`。
