...
### AI 助手强制前置检查（无论改动大小，均不得跳过）

**每次开始实现任何代码改动之前**，AI 助手必须执行以下步骤，不得以"这只是小改动"为由跳过：

1. **检查上下文中最近一次终端退出码**：
   - 若退出码已为非 0，必须先说明测试失败原因，判断是否与本次任务无关，明确后再继续。
   - 不得在未说明原因的情况下直接动手写代码。
2. **若当前测试状态未知**（上下文没有最近跑测试的记录），必须先运行 `npm run test:auto` 确认基线是否干净。
3. **改动完成后**，必须至少运行 `npm run test:i18n`（任何改动）和与改动相关的子测试（CSS/UI 改动 → `npm run test:ui`，逻辑改动 → `npm run test:api`）。
4. **若 UI 测试因本次预期改动失败**，运行 `npm run baseline:ui` 更新基线，并告知用户已更新。
